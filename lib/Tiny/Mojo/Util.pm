...
# Declared in Tiny::Mojo::Base to avoid circular require problems
sub monkey_patch { Tiny::Mojo::Base::_monkey_patch(@_) }
...
sub xml_escape {
  return $_[0] if ref $_[0] && ref $_[0] eq 'Tiny::Mojo::ByteStream';
  my $str = shift // '';
  $str =~ s/([&<>"'])/$XML{$1}/ge;
  return $str;
}
...
Tiny::Mojo::Util - Portable utility functions

=head1 SYNOPSIS

  use Tiny::Mojo::Util qw(b64_encode url_escape url_unescape);
...
L<Tiny::Mojo::Util> provides portable utility functions for L<Tiny::Mojo>.

=head1 FUNCTIONS

L<Tiny::Mojo::Util> implements the following functions, which can be imported
individually.
...
do not escape L<Tiny::Mojo::ByteStream> objects.
