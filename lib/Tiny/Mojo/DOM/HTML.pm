package Tiny::Mojo::DOM::HTML;
use Tiny::Mojo::Base -base;

use Tiny::Mojo::Util qw(html_unescape xml_escape);
...
Tiny::Mojo::DOM::HTML - HTML/XML engine
...
=head1 SYNOPSIS
  ...
  my $html = Tiny::Mojo::DOM::HTML->new;
  $html->parse('<div><p id="a">Test</p><p id="b">123</p></div>');
  my $tree = $html->tree;

=head1 DESCRIPTION

L<Tiny::Mojo::DOM::HTML> is the HTML/XML engine used by L<Tiny::Mojo::DOM>, based on the
L<HTML Living Standard|https://html.spec.whatwg.org> and the
L<Extensible Markup Language (XML) 1.0|http://www.w3.org/TR/xml/>.

=head1 ATTRIBUTES

L<Tiny::Mojo::DOM::HTML> implements the following attributes.
...
L<Tiny::Mojo::DOM::HTML> inherits all methods from L<Tiny::Mojo::Base> and implements the
following new ones.

=head2 parse

  $html = $html->parse('<foo bar="baz">I ♥ Tiny::Mojolicious!</foo>');
